...
/** Native entrypoint installs disk access; portable imports never load it. */
export interface NativeStaticAssets {
  exists(directory: string): boolean;
  readManifest(path: string): StaticAssetPackage;
  serve(request: Request, options: StaticAssetOptions, segments: string[]): Response | undefined;
}
let nativeAssets: NativeStaticAssets | undefined;
export function installNativeStaticAssets(implementation: NativeStaticAssets): void {
  nativeAssets = implementation;
}
...
export function createStaticAssetHandler(
  ...
  return async (request: Request): Promise<Response | undefined> => {
    ...
    if (!assetPackage && options.manifest) {
      ...
      } else if (typeof options.manifest === 'string' && nativeAssets) {
        try {
          assetPackage = nativeAssets.readManifest(options.manifest);
        } catch {
          // ignore parse error
        }
      }
    ...
    const dirExists =
      (options.live === true || !hasPackagedContents) &&
      (nativeAssets?.exists(options.directory) ?? false);
    const useLive = options.live ?? (dirExists && !hasPackagedContents);

    // Local dev: read live from disk
    if (useLive && dirExists) {
      return nativeAssets!.serve(request, options, segments);
    }
