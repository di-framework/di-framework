...
describe('CLI actor commands', () => {
  ...
  beforeEach(async () => {
    ...
    const { SqliteActorStorage, ActorRuntime, Actor, ActorMethod, ActorContext } = await import(
      '../../di-framework-actors/src/index.ts'
    );
  ...
  it('actor list outputs known actors in human text and json format', async () => {
    ...
    const code = await main(
      ['actor', 'list', '--dir', actorDir, '--namespace', 'cli-app'],
      textCap.io,
    );
    expect(code).toBe(0);
    ...
    const jsonCode = await main(
      ['actor', 'list', '--dir', actorDir, '--namespace', 'cli-app', '--json'],
      jsonCap.io,
    );
