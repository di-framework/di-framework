...
import { runWithProfiles } from './profiles.ts';
...
export async function loadConfig<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  return runWithProfiles(options.profiles, async () => {
 ...
 */
export function loadConfigSync<T extends Record<string, unknown> = Record<string, unknown>>(
  options: LoadConfigOptions<T> = {},
): T {
  return runWithProfiles(options.profiles, () => {
    let merged: Record<string, unknown> = { ...(options.defaults ?? {}) };
