...
export function sqlWhere(filter: Record<string, unknown> = {}): { sql: string; args: unknown[] } {
  ...
  for (const [key, value] of Object.entries(filter)) {
    const [fragment, values] = op(value);
    clauses.push(`${ident(key)} ${fragment}`);
    args.push(...values);
  }
  return { sql: clauses.length ? ` WHERE ${clauses.join(' AND ')}` : '', args };
}

export interface SqlAdapterOptions<E> extends SqlMapping<E> {
  table: string;
}
...
> implements StorageAdapter<E, ID>, ConditionalStorageAdapter<E, ID>
{
  ...
  async findById(id: ID): Promise<E | null> {
    const row = await this.firstRow(
      `SELECT * FROM ${this.table} WHERE ${this.idColumn} = ? LIMIT 1`,
      [id],
    );
  ...
  async findMany(ids: ID[]): Promise<E[]> {
    ...
    const rows = await this.allRows(
      `SELECT * FROM ${this.table} WHERE ${this.idColumn} IN (${marks})`,
      ids,
    );
    return rows.map((r) => this.fromRow(r));
  }
  async findAll(): Promise<E[]> {
    return (await this.allRows(`SELECT * FROM ${this.table}`)).map((r) => this.fromRow(r));
  }
  async save(entity: E): Promise<E> {
    ...
    const updates = keys
      .filter((k) => ident(k) !== this.idColumn)
      .map((k) => `${ident(k)} = excluded.${ident(k)}`)
      .join(',');
    const sql = `INSERT INTO ${this.table} (${cols}) VALUES (${marks}) ON CONFLICT (${this.idColumn}) DO UPDATE SET ${updates || `${this.idColumn}=excluded.${this.idColumn}`}`;
    await this.run(
      sql,
      keys.map((k) => row[k]),
    );
  ...
  async delete(id: ID): Promise<boolean> {
    const result = await this.run(`DELETE FROM ${this.table} WHERE ${this.idColumn} = ?`, [id]);
    return (result.changes ?? 0) > 0;
  }
  async count(filter: Record<string, unknown> = {}): Promise<number> {
    const w = sqlWhere(filter);
    const row = await this.firstRow(`SELECT COUNT(*) as count FROM ${this.table}${w.sql}`, w.args);
  ...
  async findPaginated(params: PaginationParams = {}) {
    ...
    const w = sqlWhere(params.filter);
    const order = (Array.isArray(params.sort) ? params.sort : params.sort ? [params.sort] : [])
      .map((s) => {
        const [k, direction = 'asc'] = s.split(':');
        return `${ident(k!)} ${direction.toLowerCase() === 'desc' ? 'DESC' : 'ASC'}`;
      })
      .join(', ');
    const total = await this.count(params.filter);
    const rows = await this.allRows(
      `SELECT * FROM ${this.table}${w.sql}${order ? ` ORDER BY ${order}` : ''} LIMIT ? OFFSET ?`,
