...
| [`cf-worker`](packages/cf-worker) | Cloudflare Worker routing and Durable Objects | `build`, `check`, `dev`, `start`, `deploy`, `cf-typegen` | Requires Wrangler; deployment requires Cloudflare authentication. It compiles sibling `services` sources, while Wrangler runs from source rather than `dist/`. |
...
| [`graphql`](packages/graphql) | Schema generation, bounded contexts, and GraphQL HTTP/WebSocket serving | `build`, `check`, `start`, `serve`, `sdl` | Run tests directly with `bun test`. `serve` uses port 4000, and its GraphiQL UI needs network access for CDN assets. Emit relies on rewriting `.ts` import extensions. |
| [`http-router`](packages/http-router) | HTTP controller and router composition | `build`, `check` | It includes sibling `services` sources, producing a nested output layout. Run tests with `bun test`. |
...
| [`services`](packages/services) | Constructor-injected application services | `build`, `check` | Run tests with `bun test`. This source is also consumed by `basic`, `http-router`, and `cf-worker`, so changes can affect all four builds. |
