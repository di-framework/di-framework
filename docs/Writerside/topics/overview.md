...
- **AI**: Annotation-driven chat, tools, RAG, MCP, and agents with `@di-framework/ai` (OpenAI-compatible and Anthropic HTTP adapters). Agent Skills (`SKILL.md`) and the skills toolbox live in `@di-framework/ai-utils`. Isolated Linux guests for untrusted code live in `@di-framework/sandboxes`.
...
- [Sandboxes](sandboxes.md) - Isolated Linux guests for untrusted code (`@di-framework/sandboxes`)
